...
class ImageAcquirer:
    ...
    def __init__(
        ...
        #
        self._timeout_for_image_acquisition = 1000000  # ms dsx old value 1
        ...
        num_buffers_default = 1 #dsx old value 16
        try:
        ...
        if file_path is not None:
            ...
            try:
                ...
                try:
                    ...
                    #     zf.extract(file_path,wkspFldr+"\\")
                    #     zf.close()
                        # file_path.replace(".zip",".xml")
                    # if file_path.endswith('.zip'):
                    #     existGDBPath = pathlib.Path(file_path)
                    #     dir_name = existGDBPath.parent
                    #     extension = ".zip"
                    #
                    ...
                    #             file_path = file_path.replace(".zip",".xml")
                    #     # print(os.listdir(dir_name))
                    #
                    # #dsx
                    print("dsx2.2", file_path)
                    node_map.load_xml_from_zip_file(file_path)
                except RuntimeException as e:
                    print("exception",e)
                    node_map.load_xml_from_file(
                        r"E:\JaiSDK\Harvester_camera\docs\JAI_AD-080GE_1_Ver214.xml")
                    print("exception end")
    ...
    def start_image_acquisition(self, run_in_background=False):
        ...
        Will be deprecated shortly.
        """
        print("run_in_background dsx",run_in_background)
    ...
    def _announce_buffers(
        ...
        for token in _buffer_tokens:
            ...
            print("announced_bufferend",announced_buffers)
